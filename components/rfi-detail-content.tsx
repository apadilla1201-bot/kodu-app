...
export function RFIDetailContent({ rfi }: { rfi: RFIData }) {
  ...
  const handleForwardPdf = async () => {
    ...
    if (emails.length === 0) {
      toast({ title: 'Missing emails', description: 'Enter at least one email address', variant: 'destructive' });
      return;
    }
    ...
    try {
      const res = await fetch(`/api/rfis/${rfi.id}/forward`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      toast({ title: 'RFI forwarded', description: `Sent to ${data?.sentTo?.length ?? 0} recipient(s)` });
      setShowForwardModal(false);
      setForwardEmails('');
      setForwardMessage('');
    } catch (err: any) {
      toast({ title: 'Error', description: err?.message ?? 'Failed to forward RFI', variant: 'destructive' });
    } finally {
      ...
      {showForwardModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl shadow-xl max-w-lg w-full p-6 space-y-4">
            <h3 className="text-lg font-semibold">Forward RFI PDF by Email</h3>
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Please review the attached RFI and respond at your earliest convenience."
                  className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm"
                />
              </div>
            </div>
