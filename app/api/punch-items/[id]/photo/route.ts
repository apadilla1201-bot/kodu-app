...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const kind = formData.get('kind') === 'completion' ? 'completion' : 'issue';
    const fileName = String(formData.get('fileName') || (file as File).name || `photo-${Date.now()}`);
    ...
    const updated = await prisma.punchItem.update({
      ...
      data:
        kind === 'completion'
          ? { completionPhotoUrl: cloud_storage_path, completionPhotoName: fileName }
          : { photoUrl: cloud_storage_path, photoName: fileName },
      include: {
    ...
    console.error('POST /api/punch-items/[id]/photo error:', error);
