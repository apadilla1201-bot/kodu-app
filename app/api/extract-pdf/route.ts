...
const EXTRACT_PROMPT = `You are a construction cost estimator. Extract ALL text and financial data from this subcontractor quote/invoice/proposal PDF.
...
export async function POST(request: Request) {
  try {
    ...
    let parsed: any;
    try {
      parsed = await askClaudeWithPdfJSON({
        prompt: EXTRACT_PROMPT,
        pdfBase64: base64String,
        maxTokens: 8000,
      });
    } catch (e: any) {
      console.error('[Extract PDF] LLM API error:', e?.message);
      return NextResponse.json({ error: 'Failed to extract PDF content' }, { status: 500 });
    }

    console.log('[Extract PDF] LLM response preview:', JSON.stringify(parsed)?.substring?.(0, 200));
