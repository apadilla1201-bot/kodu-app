...
import closeoutItems from './arena-madness-closeout.json';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Seed closeout: los 22 entregables del Excel (una sola vez por proyecto)
    const existingCloseout = await prisma.closeoutItem.count({ where: { projectId: project.id } });
    let closeoutCreated = 0;
    if (existingCloseout === 0) {
      for (const c of closeoutItems as any[]) {
    ...
    steps.push(`closeout: ${closeoutCreated} entregables sembrados (omitido si ya existían: ${existingCloseout})`);
