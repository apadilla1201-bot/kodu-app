...
export default function EditProjectPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const base = data?.error ?? 'Failed to update project';
  ...
  if (fetching) {
    return (
      <div className="max-w-[700px] mx-auto py-24 text-center text-muted-foreground text-sm">
        Loading project…
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[700px] mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-display font-bold tracking-tight mb-1">Edit Project</h1>
        <p className="text-sm text-muted-foreground mb-6">Update the project information</p>
